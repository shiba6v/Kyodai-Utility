...
table_content.each do |cell|
  ...
  if a.count != 0
    ...
    lecture_href = a.first.attributes['href'].value
    lecture_link = register_page.link_with(href: lecture_href)
    lecture_page = lecture_link.click
    lecture_detail_link = lecture_page.links.find{|item| item.text.include?("詳細")}
    lecture_detail_page = lecture_detail_link.click
    lecture_detail_page.xpath("//div/div/table").first
    lecture_place = lecture_detail_page.xpath("//div/div/table").first.xpath("//tr").select{|item| item.xpath("th").children.to_s.include?("講義室")}.first.xpath("td").children[2].to_s.delete("\n").delete("\s")
  end
  lecture_table[index/5][index%5] = [lecture_name, lecture_place]
  index += 1
end
...
lecture_table.each do |lecture_row|
  puts "======#{DAYS[day_count]}曜日======="
  time_count = 1
  lecture_row.each do |lecture|
    puts lecture[0]
    puts lecture[1]
    time_count += 1
  end
